...
<%
	...
	boolean isSessionActive = SessionHandler.isActiveSession(request,response);
	...
	//Get session values
	 if(isSessionActive)
	 {
		...
		userName = session.getAttribute("s_userName").toString() ;
		userId = session.getAttribute("s_userId").toString() ;
		userType = session.getAttribute("s_userType").toString() ;		
		
		societyId = session.getAttribute("s_societyId").toString() ;
		societyName = session.getAttribute("s_societyName").toString() ;
...
<% 
	if(!isSessionActive)
	{
		request.setAttribute("redirect_to","../user/index_new.jsp");
